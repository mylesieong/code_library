...
public class DecryptExe {

	static final String FILENAME_INI = "decryptExe.ini";
	...
	public static void main(String[] args) {
		pro = new Properties();
		try {
			String path = DecryptExe.class.getProtectionDomain().getCodeSource().getLocation().getPath(); 
			path = URLDecoder.decode(path, "UTF-8"); 
			path = path.replaceAll("%20", " ").substring(0, path.lastIndexOf('/')+1);
//			String path = "C:\\ZAUTOPAY\\payroll\\decrypt_java\\";		//for testing only
		...
		try {
			...
			fileName = pro.getProperty("FileName", "DBPBCM.DAT");
		        		...
		        		try {
		        			Runtime.getRuntime().exec("cmd /C start " + 
		        					pro.getProperty("DataTransfer", 
		        							"c:\\progra~1\\ibm\\client~1\\rfrompcb c:\\zautopay\\PAYroll\\PYUL.tfr"));
		        		} catch(Exception e) {
		        			JOptionPane.showMessageDialog(null, 
	...
	private static void foxproDecryption() {
		if(pro.getProperty("RetryFoxPro", "Y").equalsIgnoreCase("Y")) {
			try{
    			...
    			String foxProPath = pro.getProperty("FoxProDecryptPath", "C:\\ZAUTOPAY\\decrypt\\decrypt.exe");
