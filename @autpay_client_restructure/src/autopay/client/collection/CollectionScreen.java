...
public class CollectionScreen extends MainScreen implements ActionListener, FocusListener, CellEditorListener, KeyListener, WindowListener, DocumentListener, ComponentListener {
	
	private static final int DATABASE = 0, EXCEL = 1, PDF = 2;
	/*MYLES*/
	private static final int COLLECTION_ACC_MAX_LEN=20;
	/*/Myles*/
	/*MYLES: support font-size changing */
	private static final int COLLECTIONSCREEN_MINX = 590, COLLECTIONSCREEN_MINY=460;
	private static final int DEFAULT_FONT_SIZE_EN=12, DEFAULT_FONT_SIZE_CN=14;
	/*/Myles*/
	
	private String SEARCH_TEXT;
	private MainScreenModel model;
	private CollectionScreenController control;
	...
	private JMenuItem mitCollection;
	private JMenuItem mitOther;
	...
	private FileNameExtensionFilter filterCollectionDAT;
	...
	public CollectionScreen() throws NumberFormatException, ClassNotFoundException, SQLException {
		this(null);
	}
	
	public CollectionScreen(Properties config) throws NumberFormatException, ClassNotFoundException, SQLException {
		super(config);
		try {
			...
			*/
			...
			model = new MainScreenModel(new String[]{ln._TableName(), ln._TableAccount4C(), ln._TableAmount(), ln._TableReference()}, this);
			/*/MYLES*/
			control = new CollectionScreenController(model);
	...
	public void init() throws SQLException, ClassNotFoundException {
		...
		if(ln.getLanguage() == Language.CN) {
			...
			fnt14 = new Font("�s�ө���", Font.BOLD, 14);
		} else {	//defaults to English
			fntSearch = new Font("Tahoma", Font.ITALIC, 11);
			fnt14 = new Font("Tahoma", Font.BOLD, 14);
		}
		...
		mnuImport = new JMenu(ln._Import());
		mnuImport.setMnemonic('I');
		mnuFile.add(mnuImport);
		
		mitCollection = new JMenuItem(ln._Collection());
		mitCollection.addActionListener(this);
		mnuImport.add(mitCollection);
		
		mitOther = new JMenuItem(ln._Other());
		mitOther.addActionListener(this);
		mnuImport.add(mitOther);
		...
		lblTotalTxn.setBounds(12, 32, 117, 14);
		panel.add(lblTotalTxn);
		
		lblTotalAmount = new JLabel(ln._TotalDebitAmount());
		lblTotalAmount.setBounds(12, 68, 117, 14);
		...
		lblInputtedAmount.setBounds(12, 147, 117, 14);
		...
		lblInputtedRecord.setBounds(12, 111, 117, 14);
		...
		for(int i=0; i<tbl.getColumnCount(); i++) {
			...
			*/
			...
			tbl.setDefaultEditor(model.getTableModel().getColumnClass(i), new CollectionCellEditor().setTableModel(model.getTableModel()));			
			/*/Myles*/
		}
					...
					.addGroup(gl_panMain.createParallelGroup(Alignment.TRAILING, false)
						.addGroup(gl_panMain.createSequentialGroup()
							.addGroup(gl_panMain.createParallelGroup(Alignment.LEADING)
								.addComponent(panParty, GroupLayout.PREFERRED_SIZE, 260, GroupLayout.PREFERRED_SIZE)
								.addComponent(panPayment, GroupLayout.PREFERRED_SIZE, 260, GroupLayout.PREFERRED_SIZE)
						...
						.addGroup(gl_panMain.createSequentialGroup()
							...
							.addGap(17)))
					.addComponent(scp, GroupLayout.DEFAULT_SIZE, 504, Short.MAX_VALUE)
					.addContainerGap())
		);
		...
		filterCollectionDAT = new FileNameExtensionFilter(ln._BCMCollectionDatFile(), "DAT");
		...
		fchGenerate.setSelectedFile(new File(autocollectionOption.getProperty("GeneratedFileName", "crpbcm")));
	...
	@Override
	public void setInputtedRecord(int totalEmps) {
		txtInputtedRecord.setText(record.format(totalEmps));
	}

	@Override
	public void setInputtedAmount(double totalAmount) {
		...
		try {
			if(partyCode != null) {
				if(control.comparePassword(partyCode, password, true) == MainScreenModel.LOGOUT) {
					...
					/**
					 * Myles'Edit 2016-4-6: comment 1 line
					 */
					//setControlEnable(false);
					displayLoginWin();
				} else {
					...
					/**
					 * Myles'Edit 2016-4-6: Move 2 lines into if-block [Part B]
					 */
					setMenuEnable(true);
					initValue();
					/**
					 * Myles'Edit 2016-4-6: add 1 line
					 */
					this.setVisible(true);
				}
			}
			/**
			 * Myles'Edit 2016-4-6: Move 2 lines into if-block [PartA]
			 */
			//setMenuEnable(true);
			//initValue();
		} catch (Exception e) {
...
//				}
		...
		else if(a.getSource() == mitCollection) {
			try {
				...
				if(option != JOptionPane.CANCEL_OPTION) {
					...
					fchImport.addChoosableFileFilter(filterCollectionDAT);
					fchImport.removeChoosableFileFilter(filterDAT);
					fchImport.removeChoosableFileFilter(filterXLS);
					fchImport.removeChoosableFileFilter(filterXLSX);
					fchImport.setFileFilter(filterCollectionDAT);
					fchImport.setSelectedFile(new File(autocollectionOption.getProperty("FirstDownloadFileName", "BCMCRP1")));
					option = fchImport.showOpenDialog(this);
					if(option == JFileChooser.APPROVE_OPTION) {
						if(control.importCollectionFile(fchImport.getSelectedFile().getPath()) == true) {
			...
			try {
				...
				if(option != JOptionPane.CANCEL_OPTION) {
					...
					fchImport.removeChoosableFileFilter(filterCollectionDAT);
					...
					fchImport.setSelectedFile(new File(autocollectionOption.getProperty("GeneratedFileName", "crpbcm")));
					option = fchImport.showOpenDialog(this);
					if(option == JFileChooser.APPROVE_OPTION) {
	...
	private void displayLoginWin() {
		setMenuEnable(false);
		/**
		 * Myles'Edit 2016-4-6: Insert 1 line
		 */
...
//		}
	...
	public void componentResized(ComponentEvent e) {
		System.out.println("Component Resize in Collection Screen");
		int screenWidth= this.getWidth();
		double factor=(double)screenWidth/(double)COLLECTIONSCREEN_MINX; 
		...
		this.modifyFont(this.mnuImport, factor);
		this.modifyFont(this.mitCollection, factor);
		this.modifyFont(this.mitOther, factor);
	 ...
	 */
	protected class CollectionCellEditor extends MainScreen.AutoPayCellEditor{
		@Override
		public boolean stopCellEditing(){
			...
			if (columnClass == Double.class) {
				...
				boolean b=s.matches("[1-9]+[0-9]*")&&s.length()<=CollectionScreen.COLLECTION_ACC_MAX_LEN;
				if (!b){
					return  badEdit();
				}
			}
